...
Две команды (`.env` создаётся сам, `AGENT_TOKEN` генерируется автоматически,
`NODE_NAME` = hostname, файрвол настраивается через `BOT_IP`):
...
`ALLOW_IPS` — кто может стучаться в порт агента (файрвол ufw): бот и панель
Remnawave. Всем остальным порт закрывается — снаружи ноду не достать.
Можно перечислить сколько угодно IP через запятую.

`NEVER_BLOCK` по умолчанию = `43.245.225.14,82.24.110.234` (эти IP агент
никогда не банит). Переопределить: `NEVER_BLOCK=1.2.3.4,5.6.7.8`.
...
- Не выставляй порт агента (`8765`) в открытый интернет без файрвола —
  ограничь доступ IP бот-сервера (`ufw allow from <bot_ip> to any port 8765`).
