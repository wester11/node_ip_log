...
sudo CENTRAL_API_URL=https://netvoid.ru \
     ...
     BOT_IP=43.245.225.14 \
     bash install.sh
```

`NEVER_BLOCK` по умолчанию = `43.245.225.14,82.24.110.234` (IP бота/панели,
их агент никогда не банит). Переопределить: добавь `NEVER_BLOCK=1.2.3.4,5.6.7.8`.
Чтобы автоматически закрыть порт агента файрволом — добавь `BOT_IP=<IP_бота>`.
